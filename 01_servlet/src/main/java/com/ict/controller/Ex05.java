...
public class Ex05 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//서블릿의 역할은 클라이언트에서 요청한 것을 받아
		//요청에 맞는 일 처리를 하는 곳으로 전달(자바 = 순수자바 = POJO)하고
		//해당 일처리의 결과를 받아서 클라이언트에게 응답하는 역할을 함
		//이러한 결과를 받아서 클라리언트에게 전달하는 역할을 MVC 패턴에서 C에 해당됨
		//M : Model (자바) 
		...
		//이렇게 클라이원트가 필요한 요청을 처리하기 위해서 함께 전달 되는 정보를
		//'파라미터'라고 함
		
		//파라미터의 자료형은 기본적으로는 String 이거나 String[]임
		...
		//만약에 ID와 PW가 파라미터 값으로 넘어왔을 때 처리하는 방법
