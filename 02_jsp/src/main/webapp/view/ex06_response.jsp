...
<%--
	...
	- 리다이렉트 : response.sendRedirect("이동할 주소")
		**리다이렉트를 실행할 시 request, response를 새로 만듬(즉, 파라미터값을 가지고 있지 않음)
		
		=======================================================
		웹 페이지 이동
		1. a 태그를 통해서 웹 페이지 이동
		   <a href="이동할 주소">텍스트</a>
		   //파라미터값을 가지고 a태그를 통해서 웹 페이지 이동
		...
		2. 자바스크립트를 통해서 웹 페이지 이동
			...
			- history.go(숫자) : 웹 브라우저의 앞으로, 뒤로와 같은 효과를 냄
			**1과 2는 이벤트 처리를 해야만 웹 페이지가 이동 됨(즉, 현재 페이지가 보임)
			
			3. 리다이렉트 : response.sendReirect("이동할 주소");
						 기존의 request와 response는 사라짐
						 새로운 request와 response가 만들어짐
						 파라미터 정보도 사라짐
						 주소창에는 최종 주소가 보임
			4. 포워드 : request.getRequestDispacher("이동할주소").forward(request, response);
					  기존의 request와 response정보를 유지
					  파라미터 정보가 사라지지 않음
					  주소창에 최초주소가 보임
			
			**3과4는 이벤트 처리 없이 자동으로 웹 페이지가 이동됨(현재 페이지가 보이지 않음)
			
			
 --%>
 
 <p><a href="ex02_script.jsp">ex02_script.jsp(파라미터 없음)</a></p>
